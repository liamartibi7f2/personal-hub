...
const timeManagementModule = (function () {
  ...
  /* ============================================================
     ...
     ============================================================ */
  function _buildHTML() {
    var dayName = _dayName(new Date(_currentDate + 'T00:00:00'));
    var dateStr = _dateLabel(new Date(_currentDate + 'T00:00:00'));
    ...
    return '<section id="' + SECTION_ID + '" class="module-section">' +
      ...
      '<div class="hub-tm-grid-wrapper">' +
        '<div class="hub-tm-grid-header">' +
          '<div class="hub-tm-time-gutter-header"></div>' +
          '<div class="hub-tm-col-header">' +
            '<span class="hub-tm-col-day-name" id="hub-tm-col-day-name">' + dayName + '</span>' +
            '<span class="hub-tm-col-date-num" id="hub-tm-col-date-num">' + dateStr + '</span>' +
          '</div>' +
        '</div>' +
        '<div class="hub-tm-grid-scroll" id="hub-tm-grid-scroll">' +
          '<div class="hub-tm-grid-body" id="hub-tm-grid-body">' +
      ...
      '<div class="hub-tm-details-overlay" id="hub-tm-details-modal" style="display:none;" aria-hidden="true">' +
        '<div class="hub-tm-details-card">' +
          ...
          '<div class="hub-tm-details-body">' +
            ...
            '<label class="hub-tm-details-label" for="tm-details-desc">Description</label>' +
            '<textarea class="hub-tm-details-textarea" id="tm-details-desc" placeholder="Add a description…" rows="3"></textarea>' +
          '</div>' +
  ...
  function _bindGridClicks(section) {
    ...
    // ── mousemove: resize ghost (snap to 30-min intervals) ──
    mainCol.addEventListener('mousemove', function (e) {
      ...
      if (slotEl) {
        ...
        // Mouse outside slots — approximate from the main-col rect
        var mainRect = mainCol.getBoundingClientRect();
        var scrollTop = scroller ? scroller.scrollTop : 0;
        var relY = e.clientY - mainRect.top + scrollTop;
    ...
    document.addEventListener('mouseup', function () {
      ...
      if (gh) {
        // Extract end time from the ghost's time label
        var lbl = gh.querySelector('#ghost-time-label');
        var endMins = startMins + 30; // default
        if (lbl) {
  ...
  function _openBlockDetails(block) {
    ...
    // Description
    var descEl = document.getElementById('tm-details-desc');
    if (descEl) descEl.value = block.subtitle || '';
  ...
  function _bindBlockDetailsModal(section) {
    ...
    if (saveBtn) {
      saveBtn.addEventListener('click', function () {
        ...
        var descEl     = section.querySelector('#tm-details-desc');
        ...
        _timeBlocks[idx].subtitle = (descEl && descEl.value.trim()) ? descEl.value.trim() : '';
        if (activePick) {
...
/* ── VIEW SWITCHER (Day / Week / Month — UI only for now) ── */
  function _bindViewSwitcher(section) {
    ...
    switcher.addEventListener('click', function (e) {
      ...
      // UI-only placeholder: grid stays locked to Day; week/month render not yet implemented
      console.log('[TimeManagement] View switched to:', view);
    });

    // Restore saved view on render
    var savedView;
...
/* ── AUDIO SETTINGS (load from localStorage) ── */
  ...
  function _startAudioCheck() {
    _stopAudioCheck(); // Ensure no duplicate intervals
    ...
    _audioCheckInterval = setInterval(function () {
      ...
      if (currentHM === _lastMinuteProcessed) return; // Already processed this minute
      ...
      for (var i = 0; i < _timeBlocks.length; i++) {
        ...
        if (playStart && playEnd) break; // Both matched, stop scanning
      }

      if (playStart) _playAudio(_audioSettings.startSoundURL);
      if (playEnd)   _playAudio(_audioSettings.endSoundURL);
    }, 1000); // check every second but guard on minute change
  }
  ...
  function _playAudio(url) {
    ...
    try {
      ...
      a.play().catch(function () {
        // Autoplay may be blocked — that's OK, we fail silently
      });
    } catch (_) {}
  }
  ...
  function _renderBlock(block, mainCol) {
    ...
    el.innerHTML =
      ...
      '<div class="hub-tm-block-content">' +
        ...
        '<span class="hub-tm-block-subtitle">' + _escapeHtml(block.subtitle || '') + '</span>' +
      '</div>' +
